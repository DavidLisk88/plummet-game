...
// Canonical WOTD pool. SAME FILE shipped to the iOS widget extension at
// ios/App/PlummetWidget/wotd-pool.json — both consumers must produce the
// same word for a given date. Generated by `node generate-wotd-pool.js`.
// MUST be identical for every user/device so they all get the same word.
let _frozenPoolPromise = null;
let _frozenPool = null;     // Sorted array of {word, pos, definition}
let _frozenPoolHash = null; // Stable identifier for diagnostics
const FROZEN_POOL_URL = '/wotd-pool.json';
...
async function loadFrozenPool() {
    ...
    _frozenPoolPromise = (async () => {
        try {
            // No force-cache: this file is small (~120KB) and we want
            // updates to ship cleanly when we regenerate the pool.
            const resp = await fetch(FROZEN_POOL_URL);
            if (!resp.ok) throw new Error('HTTP ' + resp.status);
            const raw = await resp.json();
            // Canonical pool is an array of {word, pos, definition}.
            // Normalize to the shape the rest of this module expects:
            //   { word, definitions: [{ pos, definition }] }
            const arr = Array.isArray(raw) ? raw : Object.values(raw);
            const normalized = arr
                .filter(e => e && e.word && e.definition)
                .map(e => ({
                    word: e.word,
                    definitions: [{ pos: e.pos || '', definition: e.definition }],
                }));
            // Defensive re-sort to guarantee identical ordering with the widget
            normalized.sort((a, b) => asciiCompare(a.word, b.word));
            _frozenPool = normalized;
            _frozenPoolHash = hashString('plummet-pool-' + normalized.length + '-' +
                (normalized[0]?.word ?? '') + '-' + (normalized[normalized.length - 1]?.word ?? ''));
            console.log(`[WOTD] Canonical pool loaded: ${normalized.length} words (hash=${_frozenPoolHash})`);
            return _frozenPool;
        } catch (e) {
            console.warn('[WOTD] Failed to load canonical pool, will fall back to in-app dict:', e);
            _frozenPool = null;
            return null;
        } finally {
