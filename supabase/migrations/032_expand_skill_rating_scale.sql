-- ╔══════════════════════════════════════════════════════════════════════╗
-- ║  Migration 032 — Expand skill rating scale from 0-15K to 0-50K      ║
-- ║                                                                      ║
-- ║  GOAL: Same curve/behaviour, but higher ceilings so progression     ║
-- ║  through class thresholds takes longer (per-game bumps unchanged).  ║
-- ║                                                                      ║
-- ║  Scale factor: 50000 / 15000 = 3.333×                              ║
-- ║  New thresholds: medium 5000, high 16500, master 33000              ║
-- ╚══════════════════════════════════════════════════════════════════════╝


-- ════════════════════════════════════════════════════════════════════════
-- PART 1: Replace compute_profile_skill (0-15K → 0-50K)
-- ════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- EXPAND TO 0-50,000 SCALE (was 15,000) — same cubic curve
    v_skill := 50000.0 * POWER(v_skill / 100.0, 3.0);

    -- Activity bonus — scaled proportionally (was LN * 15)
    v_skill := v_skill + LN(1 + v_total_games) * 50;

    -- Confidence gate — 25 games (unchanged)
    v_skill := v_skill * LEAST(1.0, v_total_games / 25.0);

    -- DETERMINE CLASS (new thresholds for 0-50K scale)
    IF v_skill >= 33000 THEN v_class := 'master';
    ELSIF v_skill >= 16500 THEN v_class := 'high';
...
-- ════════════════════════════════════════════════════════════════════════
-- PART 2: Replace per-challenge skill functions (0-15K → 0-50K)
-- ════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- EXPAND TO 0-50,000 SCALE (was 15,000)
    v_skill := 50000.0 * POWER(v_internal / 100.0, 8);
...
BEGIN
    SELECT
        ...
        -- FIX: Use total_time_seconds (renamed from total_time_used_seconds in migration 030)
...
BEGIN
    ...
    skill_class := CASE
        WHEN v_skill >= 33000 THEN 'master'
        WHEN v_skill >= 16500 THEN 'high'
...
-- ════════════════════════════════════════════════════════════════════════
-- PART 3: Update class thresholds in update_ranking_for_account
-- ════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    ON CONFLICT (account_id) DO UPDATE SET
        ...
        -- New thresholds for 0-50K scale
        skill_class = CASE
            WHEN leaderboard_rankings.profile_id = EXCLUDED.profile_id
            THEN CASE
                WHEN GREATEST(leaderboard_rankings.skill_rating, EXCLUDED.skill_rating) >= 33000 THEN 'master'
                WHEN GREATEST(leaderboard_rankings.skill_rating, EXCLUDED.skill_rating) >= 16500 THEN 'high'
...
-- ════════════════════════════════════════════════════════════════════════
-- PART 4: Rescale existing data (×3.333) and recompute all rankings
-- ════════════════════════════════════════════════════════════════════════

-- 4a. Scale up stored skill_rating floors in profile_game_stats
UPDATE profile_game_stats
SET skill_rating = skill_rating * (50000.0 / 15000.0);

-- 4b. Scale up stored skill_rating in leaderboard_rankings
UPDATE leaderboard_rankings
SET skill_rating = skill_rating * (50000.0 / 15000.0);

-- 4c. Scale up stored challenge_skill_rating in challenge_leaderboards
UPDATE challenge_leaderboards
SET challenge_skill_rating = challenge_skill_rating * (50000.0 / 15000.0);

-- 4d. Recompute all rankings with the new functions
DO $$
DECLARE
    v_acct RECORD;
    v_count INTEGER := 0;
BEGIN
    ...
    LOOP
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '032 recompute: ranking failed for account %: %', v_acct.account_id, SQLERRM;
        END;
    END LOOP;
    RAISE NOTICE '032 recompute: recomputed rankings for % accounts', v_count;
END;
$$;

-- 4e. Update skill_class on challenge_leaderboards to match new thresholds
UPDATE challenge_leaderboards
SET skill_class = CASE
    WHEN challenge_skill_rating >= 33000 THEN 'master'
    WHEN challenge_skill_rating >= 16500 THEN 'high'
