-- ============================================================
-- PLUMMET — Word Runner Ranking Fix (010b)
-- Delta patch: Adds compute_word_runner_skill + rewrites
-- update_my_challenge_rankings to include word-runner block.
-- Also makes grid_size nullable for non-grid challenges.
-- Apply AFTER 010 has already been run.
-- ============================================================

-- Drop the old standalone function from the original 010 (not used)
DROP FUNCTION IF EXISTS update_word_runner_rankings(UUID);
...
-- ─── Rewrite update_my_challenge_rankings to include word-runner ───
CREATE OR REPLACE FUNCTION update_my_challenge_rankings(p_account_id UUID)
RETURNS VOID AS $$
DECLARE
    rec RECORD;
BEGIN
    -- Clear stale rows for this account
