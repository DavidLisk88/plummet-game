-- ╔══════════════════════════════════════════════════════════════════════╗
-- ║  038: Increase per-game skill bump values in record_game()          ║
-- ║                                                                     ║
-- ║  Makes games played the dominant factor in skill progression by     ║
-- ║  significantly increasing both min and max bump per game:           ║
-- ║                                                                     ║
-- ║  v_min_bump: speed-round 1.5→5.0, target-word/search/runner 1→4,  ║
-- ║              word-category 0.8→3.0, timed 0.5→2.0, sandbox 0.1   ║
-- ║  v_max_bump: speed-round 30→80, target-word/search/runner 25→65,  ║
-- ║              word-category 20→55, timed 15→45, sandbox 10         ║
-- ╚══════════════════════════════════════════════════════════════════════╝
...
BEGIN
    ...
    --    with PERFORMANCE-SCALED bump per game mode (038: increased)
    -- ════════════════════════════════════════════════════════════════
    BEGIN
        v_min_bump := CASE
            WHEN p_is_challenge AND p_challenge_type = 'speed-round'    THEN 5.0
            WHEN p_is_challenge AND p_challenge_type = 'target-word'    THEN 4.0
            WHEN p_is_challenge AND p_challenge_type = 'word-search'    THEN 4.0
            WHEN p_is_challenge AND p_challenge_type = 'word-runner'    THEN 4.0
            WHEN p_is_challenge AND p_challenge_type = 'word-category'  THEN 3.0
            WHEN p_game_mode = 'timed'                                  THEN 2.0
            ELSE 0.1
        END;

        v_max_bump := CASE
            WHEN p_is_challenge AND p_challenge_type = 'speed-round'    THEN 80.0
            WHEN p_is_challenge AND p_challenge_type = 'target-word'    THEN 65.0
            WHEN p_is_challenge AND p_challenge_type = 'word-search'    THEN 65.0
            WHEN p_is_challenge AND p_challenge_type = 'word-runner'    THEN 65.0
            WHEN p_is_challenge AND p_challenge_type = 'word-category'  THEN 55.0
            WHEN p_game_mode = 'timed'                                  THEN 45.0
            ELSE 10.0
        END;
